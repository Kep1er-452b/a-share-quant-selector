...
def build_messages(
    ...
    *,
    ...
    user_content = (
        ...
        "事件价格必须贴近当天 OHLC。"
        "必须先按 背景 -> 价量形态 -> 行为性质 -> CM意图 -> 行动/风险 的顺序判断，再命名模式或事件。"
        "证据不足时请输出 unclear，不要强行凑 Phase 或经典事件。"
        "请输出接近专业人工读图的长分析，不要只给摘要；summary_text 至少覆盖背景、事件链、价位、确认条件和失效条件。"
        "必须返回 book_judgment 对象，其中 action_bias、next_scenarios、invalidation、limitations 都必须非空。"
