...
async function loadStocks(forceReload = false) {
    ...
    state.stocksLoadingPromise = (async () => {
        ...
        do {
            const result = await apiFetch(`/api/stocks?page=${page}&per_page=${STOCKS_PAGE_SIZE}`);
            if (!result.success) {
...
function renderStockPeriodControls() {
...
function applyHaltState(message = '全部 Web 功能已停止。重启服务器后方可恢复。', mode = 'halt') {
    ...
    if (state.systemHalted) {
        ...
        if (mark && title) {
            if (isShutdown) {
                ...
                mark.textContent = 'EMERGENCY STOP';
                title.textContent = 'SYSTEM STOPPING';
            }
        }
        return;
    }
    ...
    const title = document.getElementById('halt-overlay-title');
    if (mark) {
        mark.textContent = isShutdown ? 'EXIT' : (isEmergency ? 'EMERGENCY STOP' : 'HALTED');
    }
    if (title) {
        title.textContent = isShutdown ? 'SYSTEM EXITING' : (isEmergency ? 'SYSTEM STOPPING' : 'SYSTEM HALTED');
    }
    if (sub) {
        sub.textContent = message;
    }
    ...
    if (haltBtn) {
        ...
        haltBtn.innerHTML = `<span class="halt-btn-dot"></span><span>${isEmergency ? 'STOPPING' : 'HALTED'}</span>`;
    }
    if (updateBtn) {
    ...
    if (shutdownBtn) {
        ...
        shutdownBtn.textContent = isShutdown ? 'EXITING' : 'EXIT';
    }
    ...
    document.getElementById('selection-results-headline').textContent = isShutdown
        ? '系统正在退出'
        : (isEmergency ? '事故急停，系统正在退出' : '系统已急停');
    document.getElementById('selection-results-meta').textContent = isShutdown
        ? 'EXITING'
        : (isEmergency ? 'EMERGENCY STOP' : 'HALTED');
    setCommandOutput(isShutdown ? 'SYS EXITING' : (isEmergency ? 'EMERGENCY STOP' : 'SYS HALTED'), 'error');
    updateGlobalTicker(
        isShutdown
            ? 'SYS EXITING   SERVER PROCESS WILL STOP   RESTART WEB TO USE AGAIN'
            : (isEmergency
                ? 'EMERGENCY STOP   INCIDENT LOG WRITTEN   SERVER PROCESS WILL STOP'
...
async function confirmHalt() {
    ...
    let haltMessage = '事故急停已触发。系统正在记录任务快照并退出，关闭后刷新页面会显示无法连接。';
    try {
        const result = await apiFetch('/api/emergency_stop', { method: 'POST' }, {
            allowWhenHalted: true,
            interpretHalt: false,
        });
        if (result && result.incident_path) {
            const incidentName = String(result.incident_path).split('/').slice(-2).join('/');
            haltMessage = `事故急停已触发。日志已写入 ${incidentName}，系统正在退出。`;
        }
    } catch (error) {
        console.warn('emergency stop request failed:', error);
    } finally {
        applyHaltState(haltMessage, 'emergency');
    }
}
