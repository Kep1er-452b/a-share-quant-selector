...
- 目前主入口实际支持的子命令有：`init`、`select`、`run`、`web`、`calendar`、`doctor`、`export`。
...
| `--update-first` | `python3 main.py export tqly --update-first` | 在 `export` 命令中先用 Tushare 单独更新该股票，再导出 |
| `--force-export` | `python3 main.py export 300888 --force-export` | 在 `export` 命令中跳过新鲜度校验，直接导出当前本地 CSV |
...
- 按股票代码、中文名称或拼音首字母查找单只股票
- 导出该股票本地 CSV 到 `/Users/chenxingyu/Downloads`
- 导出前用 Tushare 判断本地 CSV 是否达到最新交易日

常用命令：

```bash
python3 main.py export 300888
python3 main.py export 天齐锂业
python3 main.py export tqly
...
- 文件名格式为 `股票代码_股票名称.csv`，重复导出会覆盖 Downloads 中的同名文件。
- 默认会读取环境变量或配置文件中的默认 Tushare Token。
- 如果本地 CSV 不是最新，交互式终端会提供两个选项：
  - 先用 Tushare 单独更新这只股票，再导出
  - 直接导出当前 CSV，不管是否最新
- 非交互式终端不会自动选择，会提示你改用 `--update-first` 或 `--force-export`。
- `--update-first` 只更新这一只股票，不触发全市场更新。
- `--force-export` 不联网校验，适合你明确只想导出当前已有 CSV 的场景。

### 3.8 `doctor`

用途：

- 检查环境、配置、策略注册、本地 CSV 和 Web 安全基础项

常用命令：

```bash
python3 main.py doctor --offline
python3 main.py doctor --full-local
python3 main.py doctor --provider-smoke tushare --max-network-stocks 3
```
...
- `python3 main.py export ...`：单只股票 CSV 导出入口
